...
if [[ "true" == "${RUN_ANSIBLE}" ]]; then
    ...
    "${PYTHON_BIN}"/ansible-playbook edx_ansible.yml -i '127.0.0.1,' -c local -e "edx_ansible_source_repo=${CONFIGURATION_REPO}" -e "configuration_version=${CONFIGURATION_VERSION}"
